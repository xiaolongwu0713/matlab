...
%Inf = Inf(goodsubj,:);
%ele_path='/Users/long/My Drive/meta/gesture/EleCTX_Files/';
ele_path=strcat(meta_dir,'EleCTX_Files_2018_10_26/');
Etala=struct([]);
% pool all electrode togather
for i = 1:size(Inf, 1)
    pn = Inf(i, 1);
    Electrode_Folder=strcat(ele_path,'P',num2str(pn),'/electrodes_Final_Anatomy_wm_All.mat'); % input electrode file folder here
    %Electrode_Folder=strcat(ele_path,'P',num2str(pn),'/electrodes_Final_Norm.mat');
...
% plot all electrodes for multiple subjects
viewBrain(cortex, Etala, {'brain','electrodes'}, transp, [0,0,0]/255, view_vect);
