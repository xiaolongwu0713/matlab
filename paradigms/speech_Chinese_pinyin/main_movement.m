...
for i = 1:length(markers)
    ...
    Screen('Drawtext',w,double(cell2mat(pattern(1,markers(i)))),xc,yc,[255 0 0],[],1);
    Screen('Drawtext',w,progress,width-200,height-100,[255 255 255],[],1);
    Screen('Flip',w); 
    pause(5);
...
%save sound
if terminate==1
    foldername=['result_movement/',patient,'_',datestr(now,'yyyymmddHHMM'),'_escape'];
else
    foldername=['result_movement/',patient,'_',datestr(now,'yyyymmddHHMM')];
end
mkdir(foldername);
