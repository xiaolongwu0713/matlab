%% version 1: only one dot between covert and overt speech for all trials.
...
pahandle = PsychPortAudio('Open', [], 2, 4, [], 1);%use only one channels: left or right, both should be OK;
...
%% some parameter in the loop
interval=1; % version1: 0.7; version2:1;
terminate=0;
recordedaudio=[];
abspositions=[];
...
for i = 1:length(rdm_prompts)
    ...
    counting_down2(w,prompt,0,progress); % all grey
    pause(interval);
    counting_down2(w,prompt,1,progress); % initial green
    pause(interval);
    counting_down2(w,prompt,2,progress); % final green
    pause(interval);
    counting_down2(w,prompt,3,progress); % cyllable green
    pause(interval);
    counting_down2(w,prompt,4,progress); % cyllable green
    pause(interval);
    counting_down2(w,prompt,5,progress); % cyllable green
    pause(interval);
    counting_down2(w,prompt,6,progress); % cyllable green
    pause(interval);
    counting_down2(w,prompt,7,progress); % cyllable green
    pause(interval);
    ...
    [audiodata,absposition,dd,pp] = PsychPortAudio('GetAudioData', pahandle);
    abspositions=[abspositions absposition];
...
save(filename,'prompt_shown','abspositions');
