...
if isempty(ps_list) && strcmpi(type,'SEEG') && send_trigger
    %send_trigger=0;
...
% plug in headset for Yoga, otherwise error;(no need for dSPEECH laptop)
...
for i=1:100 % 100 sentence; size(audio_files)
    ...
    while etime(a_now, trial_begin)<15
        ...
        if strcmp(pressedKeys,'space')
            if pausing
                fprintf('Resuming');
                % prompt_shown format: timestamp, prompt;
