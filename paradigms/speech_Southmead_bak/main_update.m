...
if strcmp(strip(name),'Long')  
    ...
    if strcmpi(type,'SEEG')
        audio_folder='D:\data\speech_Southmead\audio\original\15_seconds'; %afile_tmp.folder;
    elseif strcmpi(type,'ECoG')
        audio_folder='D:\data\speech_Southmead\audio\square_wave\15_second_wavs_ONE_SQUARE_WAVE'; %afile_tmp.folder;
    end
else
    if strcmpi(type,'SEEG')
...
if send_trigger
	...
	elseif strcmpi(type,'ECoG')
        if strcmp(strip(name),'Long') 
            afile='D:\data\speech_Southmead\audio\square_wave\one_squarewave.wav'; 
        else
            afile='.\audio\square_wave\one_squarewave.wav'; 
        end
...
if  send_trigger 
	...
	if strcmpi(type,'ECoG')
        ...
        if strcmp(strip(name),'Long') 
            afile='D:\data\speech_Southmead\audio\square_wave\three_squarewaves.wav'; 
        else
            afile='.\audio\square_wave\three_squarewaves.wav'; 
        end
		[y, freq] = psychwavread(afile);
		wavedata = y';
		wavedata = [wavedata ; wavedata];
