...
[w, rect] = Screen('OpenWindow', whichScreen, black ,[0,0,1300,1000]) ;
...
if  send_trigger 
	...
	if strcmpi(type,'ECoG')
        ...
        afile='./audio/square_wave/ three_squarewaves.wav'; 
		[y, freq] = psychwavread(afile);
		wavedata = y';
		wavedata = [wavedata ; wavedata];
		PsychPortAudio('FillBuffer', pahandle, wavedata);
...
for j=1:total_sentences % 100 sentence; size(audio_files)
    ...
    %Screen('FillOval',w,[0 255 0],[xc-width/2-200,yc-1 00,xc-width/2,yc+100]);
    %  Screen('Flip',w);
    Screen('Drawtext',w,sentence,xc-width/2,yc,[255 255 255]);
