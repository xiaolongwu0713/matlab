...
address=strcat('D:/lsj/RJ_preprocessing_data/P',num2str(pn));
if ~exist(address,'dir')
    mkdir(address);
end
cd(address);

Folder=strcat('D:/lsj/RJ_preprocessing_data/P',num2str(pn),'/preprocessing');
if ~exist(Folder,'dir')
    mkdir(Folder);
end
...
for session = 1:sessionNum
    ...
    strname=strcat('D:/lsj/RJ_Raw_Data/P',num2str(pn),'/H',num2str(session),'.mat');
    load(strname, 'EEG');
    strname=strcat('D:/lsj/RJ_Raw_Data/P',num2str(pn),'/inf',num2str(session),'.mat');
    load(strname, 'accuracy', 'rtime');
    
    Fs = EEG.srate;
    data = double(EEG.data);
    triggerNum = size(EEG.event,2)-1;
    
    % The first line of information is useless data.
    triggerInf=zeros(triggerNum,3);
    for i=1:triggerNum
        logicpos=isstrprop(EEG.event(:,i+1).type,'digit');
        digpos=str2double(EEG.event(:,i+1).type(logicpos));
        
        triggerInf(i,1:2)=[EEG.event(:,i+1).latency digpos];
    end
...
%%
strname = strcat('d:/lsj/RJ_preprocessing_data/P',num2str(pn),'/preprocessing/preprocessingAll_v2.mat');
