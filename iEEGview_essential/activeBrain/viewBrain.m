...
viewstruct.enablecortexcolor = 1 %get(handles.checkbox_DP_ColorfulBrain,'value');
...
colorful_cortex = 1; %viewstruct.enablecortexcolor;%strmatch('cortexcolor', viewstruct.what2view,'exact');
if ~isempty(I) && ~colorful_cortex
...
% colorful_cortex = strmatch('cortexcolor', viewstruct.what2view,'exact');
% handles.modeltype = get(handles.mainGUIhandles.popupmenu_StandardModelType,'value');

switch handles.modeltype
    case 1
      stdpath=[handles.ConfigurationPath.root_path,'StdbrainModel/MNI'];
    case 2
     stdpath=[handles.ConfigurationPath.root_path,'StdbrainModel/FSAVERAGE'];
end

if colorful_cortex % enable cortex color
...
%     Color_Cindex=repmat((cmapstruct.basecol+[0.1,0.1,0.1]),Mm,1);
    ...
    handles.spec_std = handles.spec;
    spec_std = handles.spec;
