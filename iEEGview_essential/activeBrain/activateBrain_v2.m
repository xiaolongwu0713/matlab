...
Ss = length(subjstructs); % subject number
Mm = length(M.vert);
...
spec = handles.specspec; % ROI number
cix = 0;
for r = range, %for specified activation samples
    for a = 1 : La, %for all activated vertices
        ...
        while ~isempty(contribs),
            ...
            contribs(subjcontrIx, :) = []; % one done, remove
        end
        
        cix = cix + 1;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        C(cix) = sum(subjcontr) / sci; %the effective activation value
    ...
    if ~isempty(I),
        ...
        if ~isempty(II),
            ...
            if viewstruct.enablecortexcolor
                ...
                if ~isempty(spec) % replace the color of the non-selected region with constent value: [220,220,220], and keep the ROI
...
%                        Color_Cindex(Color_Mat.M(1).Cortex_Color(:,1)+size(Color_Mat.M(2).vert,1),:)=Color_Mat.M(1).Cortex_Color(:,2:4); 
                ...
                % plot brain with color of the ROI:
