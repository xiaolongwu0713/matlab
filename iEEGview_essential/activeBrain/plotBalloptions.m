...
ballRadius_Trielectrodes=1.5;
ballColor_Trielectrodes=[0 1 0];


%handles.electrode_view = get(handles.popupmenu_ElectrodesView,'value');% seeg/ecog/seeg+ecog
handles.electrode_view=1;

pos= tala.electrodes;
elec_different_color=1;
%if get(handles.checkbox_ColorAnalysis,'value') % color electrodes differently
if elec_different_color
    if tala.seeg_pos==size(tala.electrodes,1)
        for i=1:tala.seeg_pos
            elename=tala.electrodes_name{i};

            if contains(elename,'ctx') || contains(elename,'cortex')|| contains(elename,'Cortex')
                plotBalls(pos(i,:),'r',ballRadius,handles);
            elseif contains(elename,'WhiteMatter') || contains(elename,'wm') || contains(elename,'WM')
                plotBalls(pos(i,:),'g',ballRadius,handles);
            elseif contains(elename,'Hippocampus')
                plotBalls(pos(i,:),[0.5 0.1 1],ballRadius,handles);
            elseif contains(elename,'Amygdala')
                plotBalls(pos(i,:), [0 0.1 0.8],ballRadius,handles);
            elseif contains(elename,'Putamen')
    ...
    if tala.seeg_pos==size(tala.electrodes,1) || tala.seeg_pos==0
        ballRadius=[0.5,0.5,0.5,0.5,1.5,0.5,0.5,0.5,0.5,0.5,]; % manually set the ball radius
...
checkbox_DP_Trielectrodes=0;
if checkbox_DP_Trielectrodes   
