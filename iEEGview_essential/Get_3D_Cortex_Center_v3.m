...
R=M.rightcortex;
RV=length(R.vert);
for ii=1:3 % 3 cpm(cortical parcellation methods)
    M=struct;
    switch ii
        case 1
    ...
    for t=1:2 % two hemisphere
        if t==1 % function [vertices, label, colortable] = read_annotation(filename, varargin)
        ...
        for  i=1:CV.numEntries
            index{i}=find(BV==CV.table(i,end)); % extract vertex of a certain label
            while isempty(index{i})
                M(t).Center(i,:)=[nan,nan,nan];
                i=i+1;
                index{i}=find(BV==CV.table(i,end));
            end
            M(t).Center(i,:)=mean(M(t).vert(index{i},:)); % cortical region center
            Cindexed(index{i},:)=repmat((CV.table(i,1:3)),length(index{i}),1)/255; % assign RGB to all vertices in this region
