%% init
demo_dir='/Users/long/mydrive/matlab/iEEGview_essential/';
freesurfer_directory='/Applications/';
subject_name='demo_data';  
subjectDirectory=strcat(demo_dir,'demo_data/');%'/Users/long/tmp/P31/';
...
handles.ConfigurationPath.iEEGview_directory=demo_dir;
...
handles.ConfigurationPath.root_path=demo_dir;
handles.ConfigurationPath.stdBrainModel_directory=strcat(demo_dir,'StdbrainModel/');
handles.modeltype=1; % 1: MNI, 2:fsaverage
%handles.light
...
shellcommand='./viewMRIandCT.sh /Applications /Users/long/mydrive/matlab/iEEGview_essential/demo_data&';
system(shellcommand)
...
tmpmsg = ['Please input the electrode number of each pin (eg,16,10,10..., the numbers should be in the same order with the sequence of pin names). ','The sequence of pins is ( ' elecInfo.name{:} ' )'];
...
%% plot demo subject activation in standard brain
tmp = load(strcat(subjectDirectory,'activationData.mat'));
...
elec_Final_Norm_file=[handles.ConfigurationPath.subject_directory '/Electrodes/electrodes_Final_Norm.mat'];
elec_Final_Norm=load(elec_Final_Norm_file);
activation_std(handles,elec_Final_Norm);

%% plot random subject(s) activation in MNI std brain
% get electrodes from all subjects
all_pos={};
multiple_sids=1;
if multiple_sids
    for sid =good_sids(1:2)
    ...
    elec_Final_Norm_file=strcat(electrode_dir,'P10/electrodes_Final_Norm.mat');
    elec_Final_Norm=load(elec_Final_Norm_file);
    all_pos=elec_Final_Norm.elec_Info_Final_wm.pos';
end

StdbrainModel_path=[handles.ConfigurationPath.stdBrainModel_directory,'/MNI/MATLAB/WholeCortex.mat'];
stdmodel = load(StdbrainModel_path);
cortex = stdmodel.cortex;
clear stdmodel
talaa=struct;
talaa.electrodes=cell2mat(all_pos');
...
plotactivations_std(cortex, talaa, talaa.activations, KernelType,'test', 'tst', handles);
