...
%        R(els)=1.5;
        ...
        Y = R(els) * Y + ye;
        Z = R(els) * Z + ze;
        hold on; % 'FaceLighting', 'none': means no reflection;reflection make circles like hollow
        surf(X, Y, Z, 'FaceColor',color,'FaceLighting', 'none', 'EdgeColor', 'none','facealpha',1);
    end
end
